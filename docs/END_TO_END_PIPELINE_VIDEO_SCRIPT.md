...
* **Left Window (40% width):** Terminal / VS Code
  ...
  * Tab 1: Terminal running live backend agent commands (`python -m src.agent.run`).
  * Tab 2: Terminal running the answer validator (`python -m src.answer.validator cases/`).
...
1. **Wake Savanna:** Check TigerGraph Savanna 4.2.5 instance status (resume if idle).  
   *(Contingency: add `--offline` to backend CLI commands if cloud latency spikes; it runs instantly from validated fixtures).*
2. **Start UI:**
   ```bash
   cd ui && npm run dev
...
python -m src.agent.run --case HHG-001 --out cases/

# Terminal 2: Run fraud ring case (Act 2)
python -m src.agent.run --case HHG-014 --out cases/
